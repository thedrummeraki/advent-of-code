...
pub fn execute(input: &str) -> u32 {
    ...
    let mut valid_numbers = HashSet::<Element>::new();

    for number in numbers {
        for symbol in symbols.iter() {
            if number.is_above(&symbol)
                || number.is_below(&symbol)
                || number.is_one_same_line_as(&symbol)
            {
                if number.x_range().contains(&symbol.x()) {
                    valid_numbers.insert(number.to_owned());
                }
            }
        }
    }

    valid_numbers.iter().map(Element::value).sum()
}
